...
st.markdown(f"""
    <div style="background:#1a1a1a; border-radius:15px; padding:20px; border:1px solid #333;">
        ...
        <div style="color:#666; font-size:0.8rem;">Güncelleme: {last_upd.strftime('%H:%M | %d.%m')}</div>
    </div>
""", unsafe_allow_html=True)
...
with c2: st.metric("Haftalık Harcama", f"{int(weekly_cost)} ₺")

# --- YENİ: SON YÜKLEMELER KARTI ---
st.write("")
...
if not recharges.empty:
    with st.container():
        ...
        for _, row in recharges.head(5).iterrows(): # Son 5 yüklemeyi göster
            st.markdown(f"""
